...
namespace publishUpdateList.Common
{
    public class XMLProcess
    {
        ...
        public static string getFilePath(string XMLFile)
        {
            //return AppDomain.CurrentDomain.BaseDirectory.ToString() + XMLFile;
            return XMLFile;
        }
        ...
        private XmlDocument XMLLoad()
        {
            ...
            try
            {
                string filename = getFilePath(XMLFile);
        ...
        private static XmlDocument XMLLoad(string strPath)
        {
            ...
            try
            {
                string filename = getFilePath(strPath);
                if (File.Exists(filename)) xmldoc.Load(filename);
            }
            catch/* (Exception e)*/
            { }
        ...
        public static void Insert(string path, string node, string element, string attribute, string value)
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    doc.Load(getFilePath(path));
                }
                XmlNode xn = _revertCreateNode(doc, node);
                if (element.Equals(""))
                {
                    if (!attribute.Equals(""))
                    {
        ...
        private static XmlNode _revertCreateNode(XmlDocument doc, string strNode)
        {
            // 递归创建node
            ...
            {
                beginIndex = ++endIndex;
                for (; endIndex < strNode.Length; ++endIndex)
                {
                    if (strNode[endIndex] == '/')
                        break;
                }
                if (endIndex == beginIndex)
                    continue;
                string nodeName = strNode.Substring(beginIndex, endIndex - beginIndex);
                if (lastNode == null)
                {
                    lastNode = doc.SelectSingleNode("/" + nodeName);
                    if (lastNode == null)
                    {
                        lastNode = doc.AppendChild(doc.CreateElement(nodeName));
                    }
                    else
                    {
                        lastNode = doc.SelectSingleNode("/" + nodeName);
                ...
                {
                    if (lastNode.SelectSingleNode(nodeName) == null)
                    {
                        XmlNode curNode = doc.CreateElement(nodeName);
                        lastNode.AppendChild(curNode);
                        lastNode = curNode;
                    }
                    else
                    {
                        lastNode = lastNode.SelectSingleNode(nodeName);
                    }
                }
                if (endIndex >= strNode.Length)
                {
                    break;
                }
            }
